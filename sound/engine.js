...
(function (root) {
  ...
  function mount(env) {
    ...
    function bell(out, t, f, tau, maxLength) {
      ...
      for (const [mult, level] of [[1, 0.22], [2, 0.07], [2.76, 0.03]]) {
        const osc = ctx.createOscillator();
        osc.type = 'sine';
    ...
    function mark(s, ph) {
      ...
      if (ph.phase === 0 || ph.phase === 2) {
        bell(s.out, ph.t, f, def.bellTau ? def.bellTau(ph.T) : 0.35, ph.T);
      } else {
        blip(s.out, ph.t, f);
        blip(s.out, ph.t + 0.12, f);
      }
    }
    ...
    function pump() {
      ...
      for (;;) {
        ...
        if (nowMs - startMs < 250) {
          const next = s.timeline[(s.k + 1) % s.timeline.length];
          const ph = { phase: e.phase, t: toAudio(startMs), T: e.dur, nextPhase: next.phase, first: s.c === 0 && s.k === 0 };
          mark(s, ph);
          s.voice.phase(ph);
        }
        s.k++;
    ...
    async function start(info) {
      ...
      if (token !== startToken) return; // пока ждали, сессию остановили или перезапустили
    ...
    function stop() {
      ...
      later(() => {
        s.out.disconnect();
        if (!session && ctx.state === 'running') ctx.suspend(); // тишина — не тратим батарею
      }, 450);
    }
    ...
    function setMode(id) {
      ...
      modeId = next;
      if (session && lastInfo && !lastInfo.once) start(lastInfo);
      else stop();
    }
    ...
    function wake() {
      if (session && ctx && ctx.state !== 'running') ctx.resume().then(pump, () => {});
    }
